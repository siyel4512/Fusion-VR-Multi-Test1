...
namespace HurricaneVR.Framework.Core.Player
{
    public class HVRPlayerController : MonoBehaviour
    {
        ...
        protected virtual void HandleHorizontalMovement()
        {
            ...
            if (GetComponentInChildren<NetworkMecanimAnimator>() != null)
            {
                if (movement.magnitude > 0.5f)
                {
                    Debug.Log("Walk");
                    GetComponentInChildren<NetworkMecanimAnimator>().Animator.SetInteger("Transition", 10);
                }
                else
                {
                    Debug.Log("stop");
                    GetComponentInChildren<NetworkMecanimAnimator>().Animator.SetInteger("Transition", 0);
                }
            }
